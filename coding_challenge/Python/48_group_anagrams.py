...
def group_anagrams(items: list):
    ...
    for item in items:
        ...
        if sorted_item not in check_appear: # Đi kiểm tra xem sorted_item có trong dict hay không
            check_appear[sorted_item] = [item] # Nếu chưa thì add key = list với item đó có rồi thì tiếp tục thêm phần tử vào dict
        else:
